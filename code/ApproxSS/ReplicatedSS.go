...
func NewReplicatedSS(N, T int, params ckks.Parameters) (repSS *ReplicatedSS) {
	repSS = new(ReplicatedSS)
	repSS.T = T
	repSS.N = N

	repSS.secretGen = ckks.NewKeyGenerator(params)
...
func (repSS *ReplicatedSS) ApproxRecover(bound4smudgingNoise int) (isSuc bool, timeComp time.Duration, sizeComm float64) {
	...
	for i := big.NewInt(0); i.Cmp(repSS.numPieces) == -1; {
		...
		smudgingNoiseSampler, actualBound := CreateSecureSmudgingNoiseSampler(repSS.params, ringQ, bound4smudgingNoise)
		ni := ringQ.NewPoly()
		smudgingNoiseSampler.ReadLvl(levelQ, ni)

		// 输出安全信息（只在第一次时输出）
		if i.Cmp(big.NewInt(1)) == 0 && actualBound != bound4smudgingNoise {
			fmt.Printf("ReplicatedSS: Using secure bound %d (original: %d)\n", actualBound, bound4smudgingNoise)
		}
...
func (repSS *ReplicatedSS) ApproxRecover4TestTime(bound4smudgingNoise int) (isSuc bool, timeComp, sizeComm *big.Float) {
	...
	for i := big.NewInt(0); i.Cmp(big.NewInt(1)) == -1; {
		...
		// 输出安全信息（只在第一次时输出）
		if i.Cmp(big.NewInt(1)) == 0 && actualBound != bound4smudgingNoise {
			fmt.Printf("ReplicatedSS (TestTime): Using secure bound %d (original: %d)\n", actualBound, bound4smudgingNoise)
		}
		ringQ.Add(piece, ni, piece)
